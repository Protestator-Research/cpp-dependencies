...
class NSSConan(ConanFile):
    ...
    requires = ["nspr/4.35", "sqlite3/3.45.0", "zlib/1.3.1"]
    ...
    def build_requirements(self):
        env = Environment()
        if is_msvc: #and not env.contains("CONAN_BASH_PATH"):
            self.build_requires("msys2/cci.latest")
        if self.settings.os == "Windows":
            self.build_requires("mozilla-build/3.3")
        if hasattr(self, "settings_build"):
            self.build_requires("sqlite3/3.45.0")

    def configure(self):
        self.options["nspr/*"].shared = True
        self.options["sqlite3/*"].shared = True

        if self.options.shared and not is_msvc:
            del self.options.fPIC
    ...
    def _make_args(self):
        ...
        def _format_library_paths(library_paths, settings):
            compiler = _base_compiler(settings)
            pattern = "-LIBPATH:%s" if str(compiler) == "msvc" else "-L%s"
        ...
        def _format_libraries(libraries, settings):
            ...
            for library in libraries:
                if str(compiler) == "msvc" or str(compiler_base) == "msvc":
